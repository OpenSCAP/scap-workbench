...
skipped if you do not need the features explained in it.
...
you can load by clicking on the *tailoring file combobox* and selecting *(...)*
...
does not affect values passed to any of the checks.
...
take. The only thing that is used to estimate progress is the amount of rules
that have been processed and the amount that is remaining. Please be patient
and wait for oscap to finish evaluation.

****
You can cancel the scan at any point by clicking the *Cancel* button.
****
...
[[alternative-contents]]TODO TODO TODO
