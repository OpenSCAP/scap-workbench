...
VERSION := $(shell python src/scap_workbench/core/version.py)
...
install:
	...
	test -d $(DATADIR)/$(PKG)/dialogs || $(INSTALL) $(MODE_DIR) -d $(DATADIR)/$(PKG)/dialogs
	...
	$(INSTALL) $(MODE_REG) src/glade/dialogs/*.glade $(DATADIR)/$(PKG)/dialogs
